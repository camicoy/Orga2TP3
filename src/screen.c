...
ca save[VIDEO_FILS][VIDEO_COLS];//para el debug necesito guardarme la pantalla
...
void screen_guardar_pantalla_actual(){
	int fila, columna;
	for(fila = 0; fila<VIDEO_FILS;fila++){
		for(columna = 0; columna < VIDEO_COLS; columna++){
			save[fila][columna].c = p[fila][columna].c ;
			save[fila][columna].a = p[fila][columna].a ;
		}
	}
}

void screen_cargar_pantalla_actual(){
	int fila, columna;
	for(fila = 0; fila < VIDEO_FILS;fila++){
		for(columna = 0;columna < VIDEO_COLS; columna++){
			p[fila][columna].c=save[fila][columna].c;
			p[fila][columna].a=save[fila][columna].a;
		}	
	}
}
